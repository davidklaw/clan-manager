<?
function resizer_main($image, $dest_file_prefix, $w, $h, $name){

global $use_imagecreatetruecolor;
global $use_imagecopyresampled;
global $IMG_ROOT;
global $JPG_QUALITY;
...
if(trim($image) == "" || trim($image) =="none") return false;
	...
	if ( ($dest_file_prefix != null) && ($arr_img["w"] > 108) )
	{
		$w = "108"; $h = "*";
	}
	else if ( ($dest_file_prefix == null) && ($arr_img["h"] > 350) )
	{
		$w = "*"; $h = "350";
	}
	else
	{
		$w = $arr_img["w"]; $h = $arr_img["h"];
	}
	...
	ImageJPEG($img_res,"$IMG_ROOT/$dest_file_prefix$image_name", $JPG_QUALITY);
