...
function DisplayHeadlines($Num)
{
	...
	while( $rowk = mysql_fetch_array($resultk) )  {
		...
		if ( $news_comments == "Y" )
		{
			...
			$comments = "<a target=content href='content.php?newsid=$news_id'>$comnum</a>";
		}
		else
			$comments = "";
		...
		$template = eregi_replace("<% subject %>", "<a target=content href='content.php?newsid=$news_id'>$news_subject</a>", $template);	
		$template = eregi_replace("<% reporter %>", "<a target=content href=content.php?page=roster&ChooseRoster=$roster_id>$news_reporter</a>", $template);
...
{		
	...
	while( $row = mysql_fetch_array($result) ) {
		...
		if ( $roster_config )
			...
			$gotfrag2 = explode("=",$roster_gotfrag);
			$gotfrag2 = $gotfrag2[2];
			$gotfrag = explode("&",$gotfrag2);
			$gotfrag = $gotfrag[0];
			...
			$template = eregi_replace("<% rowClass %>", $rowClass, $template);			
			$template = eregi_replace("<% alias %>", "<a target=content href=content.php?page=roster&ChooseRoster=$roster_id>$roster_alias</a>", $template);
			...
			$template = eregi_replace("<% email %>", "<a target=content href='content.php?page=email&id=$roster_id'>email me</a>", $template);	
			...
			$template = eregi_replace("<% details %>", "<a target=content href=content.php?page=roster&ChooseRoster=$roster_id>details</a>", $template);
...
function DisplayUpcomingMatches($Num)
{
	...
	while( $row = mysql_fetch_array($recresult) )  {
		...
		$template = eregi_replace("<% opponents %>", "<a target=content href=content.php?page=records&ChooseRecord=$record_id>$record_awayteam</a>", $template);	
		$template = eregi_replace("<% tag %>", "<a target=content href=content.php?page=records&ChooseRecord=$record_id>$record_awaytag</a>", $template);	
		...
		$template = eregi_replace("<% details %>", "<a target=content href=content.php?page=records&ChooseRecord=$record_id>details</a>", $template);
...
function DisplayRecordsDetails($ChooseRecord)
{
	...
	for( $counter=0; $counter < sizeof($players); $counter++ ) {
		...
		if ( $rowx = mysql_fetch_array($resultx) ) {
			...
			$listplayers .= "<a target=content href='content.php?page=roster&ChooseRoster=$roster_id'>$players[$counter]</a>";
		}
		else {
	...
	while( $rowg = mysql_fetch_array($resultg) )
	{
		...
		$demodisplay .= "<tr><td class='RecordDemosCell'>".$demo_awayteam."</td>
									 ...
									 <td class='RecordDemosCell'><a target=content href='content.php?page=demos&ChooseDemo=".$demo_id."'>view</a></td></tr>";
	}
	$demodisplay .= "</table>";
...
function DisplayFileList($sort,$scend)
{
	...
	while( $row = mysql_fetch_array($result)) {
		...
		if ( $file_external ) {
			...
			$template = eregi_replace("<% filename %>", "<a target=content href='content.php?page=files&ChooseFile=$file_id'>$external_name[$size]</a>", $template);
			$template = eregi_replace("<% filesize %>", "n/a", $template);	
		}
		else {
			$template = eregi_replace("<% filename %>", "<a target=content href='content.php?page=files&ChooseFile=$file_id'>$file_name</a>", $template);
		...
		$template = eregi_replace("<% details %>", "<a target=content href=content.php?page=files&ChooseFile=$file_id>download</a>", $template);
...
function DisplayLinksList($sort,$scend)
{
	...
	while( $row = mysql_fetch_array($result)) {
		...
		$template = eregi_replace("<% rowClass %>", $rowClass, $template);
		$template = eregi_replace("<% linkname %>", "<a target=content href='content.php?page=links&ChooseLink=$link_id'>$link_name</a>", $template);
		...
		$template = eregi_replace("<% details %>", "<a target=content href=content.php?page=links&ChooseLink=$link_id>view</a>", $template);
...
{	
	...
	while( $row = mysql_fetch_array($result)) {
		...
		$template = eregi_replace("<% rowClass %>", $rowClass, $template);
		$template = eregi_replace("<% sponsorname %>", "<a target=content href='content.php?page=sponsors&ChooseSponsor=$sponsor_id'>$sponsor_name</a>", $template);
		...
		$template = eregi_replace("<% sponsorimage %>", "<a target=content href='content.php?page=sponsors&ChooseSponsor=$sponsor_id'>$image</a>", $template);		
		$template = eregi_replace("<% details %>", "<a target=content href='content.php?page=sponsors&ChooseSponsor=$sponsor_id'>view</a>", $template);
...
function DisplayRecentEvents($Num)
{
	...
	while( $row = mysql_fetch_array($everesult) )  {
		...
		$template = eregi_replace("<% rowClass %>", $rowClass, $template);
		$template = eregi_replace("<% eventname %>", "<a target=content href='content.php?page=events&ChooseEvent=$event_id'>$event_name</a>", $template);
		...
		$template = eregi_replace("<% eventimage %>", "<a target=content href='content.php?page=events&ChooseEvent=$event_id'>$image</a>", $template);		
		$template = eregi_replace("<% details %>", "<a target=content href='content.php?page=events&ChooseEvent=$event_id'>details</a>", $template);
...
{	
	...
	while( $row = mysql_fetch_array($result) ) {
		...
		$template = eregi_replace("<% rowClass %>", $rowClass, $template);
		$template = eregi_replace("<% servername %>", "<a target=content href='content.php?page=servers&ChooseServer=$server_ip'>$server_name</a>", $template);
		...
		$template = eregi_replace("<% details %>", "<a target=content href='content.php?page=servers&ChooseServer=$server_ip'>details</a>", $template);
...
{	
	...
	while( $row = mysql_fetch_array($demresult) )  {
		...
		if ( $demo_match != null )
		{
			$resul2 = connection("SELECT record_awayteam,record_awaytag FROM records WHERE record_id='$demo_match'");
			$ro2 = mysql_fetch_array($resul2); extract($ro2);
			
			$template = eregi_replace("<% demoteam %>", "<a target=content href='content.php?page=demos&ChooseDemo=$demo_id'>$record_awayteam</a>", $template);
			$template = eregi_replace("<% demotag %>", "<a target=content href='content.php?page=demos&ChooseDemo=$demo_id'>$record_awaytag</a>", $template);
		...
		$template = eregi_replace("<% demomatch %>", "<a target=content href='content.php?page=records&ChooseRecord=$demo_id''>$demo_match</a>", $template);
		...
		$template = eregi_replace("<% details %>", "<a target=content href='content.php?page=demos&ChooseDemo=$demo_id'>view</a>", $template);
...
function DisplayDemosDetails($ChooseDemo)
{
	...
	$template = eregi_replace("<% demopov %>", "$demo_pov", $template);			
	$template = eregi_replace("<% demomatch %>", "<a target=content href='content.php?page=records&ChooseRecord=$demo_match''>view match</a>", $template);
	$template = eregi_replace("<% demoevent %>", "$demo_event", $template);
	$template = eregi_replace("<% demosize %>", "$demo_size", $template);
	$template = eregi_replace("<% democomment %>", "$demo_comment", $template);
	$template = eregi_replace("<% demoposter %>", "$demo_poster", $template);
...
{	
	...
	while( $row = mysql_fetch_array($result)) {
		...
		$template = eregi_replace("<% webmaster %>", "<a target=content href='content.php?page=email&id=$webmaster'>$webmaster</a>", $template);
		$template = eregi_replace("<% manager %>", "<a target=content href='content.php?page=email&id=$manager'>$manager</a>", $template);
		$template = eregi_replace("<% scheduler %>", "<a target=content href='content.php?page=email&id=$scheduler'>$scheduler</a>", $template);
		$template = eregi_replace("<% recruiting %>", "<a target=content href='content.php?page=email&id=$recruiting'>$recruiting</a>", $template);
		$template = eregi_replace("<% help %>", "<a target=content href='content.php?page=email&id=$help'>$help</a>", $template);
		$template = eregi_replace("<% marketing %>", "<a target=content href='content.php?page=email&id=$marketing'>$marketing</a>", $template);
...
{	
	...
	while( $row = mysql_fetch_array($result)) {
		...
		$template = eregi_replace("<% rowClass %>", $rowClass, $template);
		$template = eregi_replace("<% gallery %>", "<a target=content href='content.php?page=screenshots&ChooseGallery=$screen_gallery'>$gallery_name</a>", $template);		
		...
		$template = eregi_replace("<% view %>", "<a target=content href='content.php?page=screenshots&ChooseGallery=$screen_gallery'>view</a>", $template);
